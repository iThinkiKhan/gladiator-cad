...
rear ends of the long deck slots as its primary M2 anchors and the rear M2 pair
...
# rim's Z20 top surface. Broad flared brackets wrap into offset rear columns,
# replacing the earlier thin doglegs while retaining open screwdriver paths.
arm_z1 = FLOOR_Z + 3.0
...
cradle = cradle.fuse(box(18.4, 32.7, 127.4, 134.0, 2.2, TAB_TOP))
cradle = cradle.fuse(box(46.3, 60.6, 127.4, 134.0, 2.2, TAB_TOP))
# Four-millimetre rear columns are offset from the M2 service screws.
cradle = cradle.fuse(box(28.7, 32.7, 127.4, 134.0, TAB_TOP, arm_z1))
cradle = cradle.fuse(box(46.3, 50.3, 127.4, 134.0, TAB_TOP, arm_z1))
# Full-width flares overlap the rails for several millimetres, then wrap around
# the offset columns. Their 5.2 mm vertical section acts as an integral gusset.
...
right_flare = [(54.0, 123.0), (49.9, 123.0), (46.3, 127.4),
               ...
               (54.0, 126.0)]
cradle = cradle.fuse(plan_prism(left_flare, 20.0, arm_z1))
cradle = cradle.fuse(plan_prism(right_flare, 20.0, arm_z1))
cradle = cradle.removeSplitter()
